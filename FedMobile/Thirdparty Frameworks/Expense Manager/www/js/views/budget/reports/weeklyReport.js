define(['jquery', 'underscore', 'Backbone'
        ,'text!views/budget/reports/weeklyReport.tpl'
...
function ($, _, Backbone
	...
	var quaterlyReport = Backbone.View.extend({	 		
        ...
        renderSuccess:function(){
        	var expenceAndBudgetList = els.get("reportWeeklyTxn");
        	var budg=0;
        	var expc=0;
        	var monthval="";
        	...
        	this.$el.html(_.template(weeklyReportTemplate,{expc:expc,budg:budg,monthval:monthval})).i18n();
        ...
        // Prepare Data - BAR and LINE Chart
			...
			if(expenceAndBudgetList.length>0)
			{
					...
					var tempBudg=0;
					for(var i=0;i<expenceAndBudgetList.length;i++){
						if(expenceAndBudgetList[i].expenceDescription=="OverAllWeaklyExpense"){
							if(!(expenceAndBudgetList[i].expences=="null" || expenceAndBudgetList[i].expences==null || expenceAndBudgetList[i].expences=="")){
									...
									var weekName = getDateOfISOWeek(parseInt(expenceAndBudgetList[i].periodValue),parseInt(expenceAndBudgetList[i].year_value));
									ticks.push(weekName);
