define(['jquery', 'underscore', 'Backbone'
        ,'text!views/budget/reports/quaterlyReport.tpl'
        , 'collections/budget/reportsCollections'
    ],
        
function ($, _, Backbone
			, quaterlyReportTemplate
			, reportsCollections
		){
	...
	var quaterlyReport = Backbone.View.extend({	 		
        ...
        renderSuccess:function(){
        	$(".back_butt").show();
        	var expenceAndBudgetList = els.get("reportQuartelyTxn");
        	var budg=0;
        	var expc=0;
        	var monthval="";
        	if(expenceAndBudgetList !=null){
	        	if(expenceAndBudgetList.length>0){
	        		...
	        		//monthval = expenceAndBudgetList[0].periodValue + "\n" + expenceAndBudgetList[0].year_value;
        	...
        	this.$el.html(_.template(quaterlyReportTemplate,{expc:expc,budg:budg,monthval:monthval})).i18n();
        ...
        // Prepare Data - BAR and LINE Chart
			...
			console.log("expenceAndBudgetList.length : " + expenceAndBudgetList.length);
			if(expenceAndBudgetList.length>0)
			{
					var s1=[];
					var s2= new Array();
					...
					for(var i=0;i<expenceAndBudgetList.length;i++){
						if(expenceAndBudgetList[i].expenceDescription=="OverAllQuartlyExpense"){
							if(!(expenceAndBudgetList[i].expences=="null" || expenceAndBudgetList[i].expences==null || expenceAndBudgetList[i].expences=="")){
								...
								var yr=expenceAndBudgetList[i].periodValue+"";
								//yr = yr.substring(2,4);
								ticks.push(yr);
